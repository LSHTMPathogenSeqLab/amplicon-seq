...
def main(args):
    ...
    with open("bam_list.txt","w") as O:
        ...
        run_cmd("freebayes -f %(ref)s -t %(bed)s -L bam_list.txt --haplotype-length -1 --min-coverage 50 --min-base-quality %(min_base_qual)s --gvcf --gvcf-dont-use-chunk true > combined.genotyped.vcf")
        run_cmd("bcftools view -T %(bed)s combined.genotyped.vcf | bcftools norm -f %(ref)s | bcftools sort -Oz -o combined.genotyped.vcf.gz")
        #run_cmd("freebayes -f %(ref)s -t %(bed)s -L bam_list.txt --haplotype-length -1 --min-coverage 50 --min-base-quality %(min_base_qual)s --gvcf --gvcf-dont-use-chunk true | bcftools view -T %(bed)s | bcftools norm -f %(ref)s | bcftools sort -Oz -o combined.genotyped.vcf.gz 2> errlog" % vars(args))
        ...
        run_cmd("bcftools query snps.vcf.gz -f '[%SAMPLE\t%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t%GT\t%TGT\t%DP\t%AD\t%TBCSQ\n]' > combined_genotyped_filtered_formatted.snps.trans.txt")    
